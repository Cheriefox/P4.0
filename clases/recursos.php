...
class Recurso{
 ...
 function guardar(){  // creae cartel
   ...
   $sql="insert into recursos(id_tecnologia,nombre,contenido,ordenamiento,activo,link_referencia)
 ...
 function actualizar($nro=0)	// actualiza cartel
	{
			...
			$sql="update recursos set id_tecnologia='$this->id_tecnologia', nombre='$this->nombre',contenido='$this->contenido'
...
function traer_datos($nro=0) // declara el constructor, si trae el numero de persona lo busca 
 ...
 static function buscar($str){
    $sql="select * from recursos where nombre like '%$str%' or contenido like '%$str%' or link like '%$str%' or id_recurso='$str' ";
    //$rs=mysql_query($sql);
	$objConn = new Conexion();
